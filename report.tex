...
This report describes the implementation and evaluation of a parallel ecosystem simulator developed for this project. 
The objective was to model interactions between rabbits, 
foxes and rocks on a 2D grid and to analyse the effect 
of introducing OpenMP-based parallelism. The document 
provides a concise overview of the algorithm in question, the adopted 
parallelisation strategy, performance results, and the main 
...
\subsection{Implementation Details and Functions}
\begin{itemize}
    \item \texttt{init\_grids}: Initialises the two grids and allocates memory for the lock table.
    \item \texttt{destroy\_grids}: Frees the allocated memory for the grids and lock table.
    \item \texttt{get\_adjacent\_index}: Retrieves the indices of adjacent cells for a given cell, considering grid boundaries.
    \item \texttt{solve\_rabbit\_conflict}: Resolves conflicts between rabbits in the same cell. 
    \item \texttt{solve\_fox\_conflict}: Does pretty much the same as the previous one, but for foxes.
\end{itemize}
...
\begin{tabular}{c|cc}
\hline
\textbf{Workers $p$} & \textbf{local $S_{\text{seq}}(p)$} & \textbf{hyrax64 $S_{\text{seq}}(p)$} \\
\hline
1  & 0.553 & 0.578 \\
2  & 0.794 & 0.816 \\
4  & 0.911 & 1.289 \\
...
\hline
\end{tabular}
\caption{Comparison between local machine and hyrax64 sequential speedup values.}
\label{tab:old-vs-new-speedup}
\end{table}
...
The differences between the two sets of results arise 
from variations in system architecture and runtime 
conditions. The earlier machine exhibited stronger 
effects from lock contention, memory bandwidth limitations, 
and cache interference, which severely hindered scalability. 
In contrast, \texttt{hyrax64} offers more efficient parallel 
execution, improved cache performance, and lower contention, 
allowing the algorithm to achieve significantly better 
speedups and more consistent internal scalability.
